...
class MachineCom(object):
	...
	def _monitor(self):
		...
		if self._grbl:
			# The longest time the CNC head could take to clear its buffered moves
			timeoutAllowance = 30
		else:
			timeoutAllowance = 5
		...
		while True:
			try:
				...
				### Printing
					...
					# Command is unknown to grbl
					if self._grbl and line.startswith("error:"):
						line = 'ok'
	...
	def roundAllFloats(self, line, precision):
		...
		with self._sendNextLock:
			...
			if not self.isStreaming():
				try:
					if self._trimFloats:
						if type(line) is str:
							line = self.roundAllFloats(line,self._floatPrecision)
